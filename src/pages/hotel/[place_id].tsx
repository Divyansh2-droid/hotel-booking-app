...
export default function HotelDetail() {
  ...
  return (
    <main className="container mx-auto p-6 max-w-5xl font-sans bg-white min-h-screen">
      <h1 className="text-4xl font-bold mb-4 text-indigo-700">{hotel.name}</h1>
      <p className="text-gray-700 mb-6">{hotel.vicinity}</p>
          ...
          {hotel.types?.length ? (
            hotel.types
              ...
              .map((type) => (
                <div
                  key={type}
                  className="flex items-center space-x-2 bg-indigo-100 text-indigo-700 px-3 py-1 rounded shadow-sm"
                >
      ...
      {/* Booking Form */}
      <section className="mb-12 px-6 py-8 bg-indigo-50 rounded-lg shadow-lg max-w-md">
        <h2 className="text-3xl font-bold mb-6 text-indigo-700">Book This Hotel</h2>
        {bookingError && <p className="text-red-600 mb-4 font-semibold">{bookingError}</p>}
        <form onSubmit={handleBooking} className="flex flex-col space-y-4">
          ...
          <button
            ...
            className="bg-indigo-700 text-white font-semibold py-2 rounded hover:bg-indigo-800 transition"
          >
